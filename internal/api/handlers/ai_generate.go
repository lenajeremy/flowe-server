	...
	return []map[string]any{
		...
		},
		{
			...
			"dataFields": map[string]any{
				...
				"codingAgentToolGrants":         "array – exact least-privilege grants for other nodes: [{nodeId, allowedOperations: string[], allowedOverrideFields: string[]}]. Empty means no tools. Read-only operations with pinned fields are the safe default; enable writes and fields only when the task requires them. The server revalidates every call and owner-approves writes",
				"codingAgentAllowWrite":         "boolean – allow repository file changes; false makes the agent read-only",
			},
			"handles": map[string]any{"inputs": []string{"target (left)"}, "outputs": []string{"source (right)"}},
			"notes":   "Never put tokens in node data or prompts. The sandbox holds no integration credentials — external work happens only through codingAgentToolGrants, which freeze exact node operations and editable fields when the job starts. Writes require owner approval. Returns a durable job result plus retained Git status, patch and changed-file artifacts.",
		},
		{
...
- codingAgentToolGrants refer to REAL node ids on the same canvas. When the task needs an external action such as creating a branch, committing files, opening a pull request or posting a message, add separately configured integration nodes for those operations, keep those tool-only nodes disconnected from the executable graph, and grant only their exact operation plus the fields Codex must supply. Pin repository, project, channel and other destination fields whenever possible. Never connect the same write node into the graph and also grant it as a tool, because that can execute the side effect twice.
