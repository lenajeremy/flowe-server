	...
	"input_schema": map[string]any{
		...
		"properties": map[string]any{
			"provider": map[string]any{
				"type":        "string",
				"enum":        []string{"gmail", "googlecalendar", "googledrive", "googledocs", "googlesheets", "googleslides", "googleforms", "googlemeet", "googlechat", "googletasks", "googlekeep", "outlook", "slack", "notion", "linear", "github", "gitlab", "monday", "asana", "jira", "confluence", "bitbucket", "stripe", "shopify", "granola", "resend", "sendgrid", "kit", "airtable", "clickup", "typeform", "calendly", "dropbox", "netlify", "vercel", "supabase", "gumroad", "googlesearchconsole", "googlecontacts", "hubspot", "front", "sentry"},
	...
	return []map[string]any{
		...
		},
		{
			...
			"dataFields": map[string]any{
				...
				"codingAgentToolGrants":         "array – one least-privilege grant per integration type: [{nodeType, nodeIds: string[], allowedOperations: string[], allowedOverrideFields: string[]}]. nodeIds is the closed allowlist of configured nodes/resources that may back the shared abilities. Empty means no tools. The server revalidates every call and owner-approves writes",
				"codingAgentAllowWrite":         "boolean – allow repository file changes; false makes the agent read-only",
			},
			"handles": map[string]any{"inputs": []string{"target (left)"}, "outputs": []string{"source (right)"}},
			"notes":   "Never put tokens in node data or prompts. The sandbox holds no integration credentials — external work happens only through codingAgentToolGrants, which freeze integration abilities, editable fields and an exact backing-resource allowlist when the job starts. Writes require owner approval. Returns a durable job result plus retained Git status, patch and changed-file artifacts.",
		},
		{
		...
		{
			"type": "sentry", "label": "Sentry", "category": "Integrations",
			"description": "Sentry: search and read issues and their latest event with the full stack trace, triage them (resolve, archive, reopen, assign, comment, delete), list projects and issue alert rules, list and create releases and deploys, and run Discover queries over events.",
			"dataFields":  map[string]any{"integrationOp": "'list_projects'|'get_project'|'list_issues'|'get_issue'|'get_latest_event'|'list_issue_events'|'list_issue_tag_values'|'resolve_issue'|'ignore_issue'|'unresolve_issue'|'assign_issue'|'delete_issue'|'list_comments'|'add_comment'|'list_releases'|'create_release'|'create_deploy'|'query_events'|'list_alert_rules'", "sentryProject": "project slug from list_integration_resources", "sentryIssueId": "numeric issue id", "sentryQuery": "Sentry search syntax, e.g. 'is:unresolved level:error'; defaults to is:unresolved for list_issues", "sentryStatsPeriod": "1h | 24h | 14d | 90d", "sentryEnvironment": "string; required by create_deploy", "sentrySort": "date | freq | new | user", "sentryLimit": "number, max 100", "sentryCursor": "pagination cursor from a previous call", "sentryStatus": "override the status a triage op sets, e.g. resolvedInNextRelease", "sentryAssignee": "user:<id> | team:<id> | an email address", "sentryComment": "comment text for add_comment", "sentryTagKey": "tag name for list_issue_tag_values, e.g. browser", "sentryIgnoreMinutes": "number – archive until this many minutes pass", "sentryIgnoreCount": "number – archive until it happens this many more times", "sentryVersion": "release version for create_release / create_deploy", "sentryProjects": "comma-separated project slugs for a release or deploy", "sentryRef": "commit sha for create_release", "sentryUrl": "link to the release or deploy", "sentryDeployName": "string", "sentryFields": "comma-separated Discover fields, e.g. 'title,count(),project'"},
			"auth":        "OAuth connection used automatically — never set integrationToken. The organization comes from the connection, so never ask for it. sentryIssueId is the numeric id, not the short id like BACKEND-4F; from an App Trigger it is {{<triggerNodeId>.data.id}}. ignore_issue archives forever unless sentryIgnoreMinutes or sentryIgnoreCount is set, which is rarely what someone automating triage wants. get_latest_event is what carries the stack trace — get_issue returns only the summary, so a workflow that asks an LLM to diagnose a crash needs get_latest_event. Triage operations need the integration to have write permission on issues; a 403 means the Sentry integration was installed with read-only access.",
			"handles":     map[string]any{"inputs": []string{"target (left)"}, "outputs": []string{"source (right)"}},
		},
		{
...
- Default to the least authority the task needs: no repository writes for explanation, investigation, review or planning, and no granted tool nodes beyond the ones the task actually uses. The agent acts on whatever text its trigger handed it — an issue body, an inbox — so every authority granted is one that text can reach.
- codingAgentToolGrants contain at most one grant per integration type. nodeIds must refer to REAL nodes of that type on the same canvas and form the closed resource allowlist beneath the shared abilities. When the task needs an external action, add separately configured integration nodes, keep those tool-only nodes disconnected, group same-type nodes into one grant, and allow only the exact operations and fields Codex needs. Pin repository, project, channel and other destinations whenever possible. Never connect the same write node into the graph and also grant it as a tool, because that can execute the side effect twice.
- A coding agent asked to open a pull or merge request MUST have disconnected backing nodes for all three provider actions: create the remote branch, commit the changed files, and open the review request. Choose the exact current operations and fields from nodeCatalog rather than guessing identifiers. Put all same-provider node IDs in one codingAgentToolGrants entry, grant those exact operations, and allow the agent to override the branch, file/content, commit-message, title, and description fields it must populate. Without these backing nodes the agent can only prepare a local patch, so never describe such a workflow as creating a PR or MR.
- Codex credentials are connected by the user in the node settings.
